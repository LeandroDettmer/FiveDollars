...
export function GitTab({ isActive, version }: GitTabProps) {
  ...
  useEffect(() => {
    ...
    invoke<{ current: string; all: string[] }>("list_git_branches", { repoPath: gitRepo.path })
      .then((b) => { if (!cancelled) setGitBranches(b); })
      .catch(() => { if (!cancelled) setGitBranches(null); });
    return () => { cancelled = true; };
  }, [isActive, gitRepo?.path]);
  ...
  const handleDetectGitRepo = async () => {
    ...
    setGitLoading(true);
    try {
      const info = await invoke<{ path: string; branch: string; is_clean: boolean; has_fivedollars_folder: boolean; has_collections_file: boolean }>("detect_git_repo", { path: selectedPath });
      setGitRepo({ path: info.path, branch: info.branch, isClean: info.is_clean, hasFivedollarsFolder: info.has_fivedollars_folder, hasCollectionsFile: info.has_collections_file });
  ...
  const handleSelectRepo = async (path: string) => {
    ...
    setGitError(null);
    try {
      const info = await invoke<{ path: string; branch: string; is_clean: boolean; has_fivedollars_folder: boolean; has_collections_file: boolean }>("detect_git_repo", { path });
      setGitRepo({ path: info.path, branch: info.branch, isClean: info.is_clean, hasFivedollarsFolder: info.has_fivedollars_folder, hasCollectionsFile: info.has_collections_file });
      const branches = await invoke<{ current: string; all: string[] }>("list_git_branches", { repoPath: info.path });
      setGitBranches(branches);
    } catch (e) {
  ...
  const handleSelectBranch = async (branch: string) => {
    ...
    try {
      ...
      const info = await invoke<{ path: string; branch: string; is_clean: boolean; has_fivedollars_folder: boolean; has_collections_file: boolean }>("detect_git_repo", { path: gitRepo.path });
      setGitRepo({ path: info.path, branch: info.branch, isClean: info.is_clean, hasFivedollarsFolder: info.has_fivedollars_folder, hasCollectionsFile: info.has_collections_file });
    ...
    <>
      <div className="about-author-panel">
        ...
        {!gitRepo && (
          <div className="about-update-section">
            ...
            {gitError && <p className="about-update-error" role="alert">{gitError}</p>}
          </div>
        )}
        {gitRepo && (
          <div className="about-update-section">
            <div className="git-repo-branch-row" style={{ display: "flex", flexDirection: "column", gap: "10px", marginBottom: "12px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                <label className="git-profile-label" style={{ minWidth: "80px" }}>{t("git.labelRepository")}</label>
                <select
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
                <label className="git-profile-label" style={{ minWidth: "80px" }}>{t("git.labelBranch")}</label>
                <select
                  className="git-select"
                ...
                >
                  {(gitBranches?.all?.length ? gitBranches.all : [gitRepo.branch]).map((b) => (
                    <option key={b} value={b}>{b === gitRepo.branch ? `${b}${t("git.currentBranchSuffix")}` : b}</option>
                  ))}
                </select>
              </div>
            </div>
            ...
            {syncedCollections.length === 0 && collectionsMode === "offline" && (
              <p style={{ fontSize: "12px", color: "var(--text-muted, #888)", margin: "4px 0 12px" }}>
                {t("git.loadProfileHint")}
              </p>
            )}
            <div style={{ marginTop: "12px", display: "flex", gap: "8px", flexWrap: "wrap" }}>
              <button type="button" className="btn-secondary" onClick={() => setGitConfirmAction("load")} disabled={gitLoading || !gitRepo.hasCollectionsFile}>
              ...
              <button
                ...
                onClick={() => { setCommitMessage(t("git.commitMessagePlaceholder")); setGitConfirmAction("saveAndCommit"); }}
                disabled={gitLoading}
              >
            ...
            {gitSyncStatus && (
              <p style={{ marginTop: "12px" }}>
                <strong>{t("git.lastAction")}</strong>{" "}
                {gitSyncStatus.lastAction ?? t("git.noActionYet")}{" "}
      ...
      {gitConfirmAction === "save" && (
        <ConfirmModal title={t("git.confirmSaveTitle")} message="" confirmLabel={t("git.confirmSaveLabel")} onConfirm={() => handleSaveToRepo(false)} onClose={() => setGitConfirmAction(null)}>
          <p style={{ margin: "0 0 8px" }}>
            {t("git.confirmSaveP1Before")} <strong>{activeProfileLabel}</strong> {t("git.confirmSaveP1Middle")}{" "}
            <code>.fivedollars/collections.json</code> {t("git.confirmSaveP1After")}
          </p>
          <p style={{ margin: "0" }}>{t("git.confirmSaveP2")}</p>
        </ConfirmModal>
      )}
      {gitConfirmAction === "saveAndCommit" && (
        <ConfirmModal title={t("git.saveAndCommit")} message="" confirmLabel={t("git.confirmSaveCommitLabel")} onConfirm={() => handleSaveToRepo(true, commitMessage)} onClose={() => setGitConfirmAction(null)}>
          <p style={{ margin: "0 0 8px" }}>
            {t("git.confirmSaveCommitP1Before")} <strong>{activeProfileLabel}</strong> {t("git.confirmSaveCommitP1Middle")}{" "}
            <code>.fivedollars/collections.json</code> {t("git.confirmSaveCommitP1After")}
          </p>
